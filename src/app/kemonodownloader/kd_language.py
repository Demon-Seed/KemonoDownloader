    ...
    def __init__(self):
        ...
        self.translations = {
            ...
            "folder_name_template": {
                "english": "Folder Name Template",
                "japanese": "フォルダ名テンプレート",
                "korean": "폴더 이름 템플릿",
                "chinese-simplified": "文件夹名称模板",
            },
            "tmpl_postid_posttitle_folder": {
                "english": "{post_id}_{post_title}",
                "japanese": "{post_id}_{post_title}",
                "korean": "{post_id}_{post_title}",
                "chinese-simplified": "{post_id}_{post_title}",
            ...
            "folder_name_template_help_title": {
                ...
                "korean": "폴더 이름 템플릿 도움말",
                "chinese-simplified": "文件夹名称模板 帮助",
            },
            "folder_name_template_help_text": {
                "english": "You can use placeholders in the folder name template:\n{post_title}, {post_id}, {creator_name}, {creator_id}\nExample: {post_title} (drops the post ID from the folder name).\nIf a template errors, the fallback {post_id}_{post_title} will be used.\nNote: this only applies when Folder Structure is set to 'Per-post folders'.",
                "japanese": "フォルダ名テンプレートではプレースホルダを使用できます:\n{post_title}, {post_id}, {creator_name}, {creator_id}\n例: {post_title}（フォルダ名から投稿IDを除外します）。\nテンプレートでエラーが発生した場合、フォールバック {post_id}_{post_title} が使用されます。\n注: これはフォルダ構造が「投稿ごとのフォルダ」に設定されている場合にのみ適用されます。",
                "korean": "폴더 이름 템플릿에서 자리 표시자를 사용할 수 있습니다:\n{post_title}, {post_id}, {creator_name}, {creator_id}\n예: {post_title} (폴더 이름에서 게시물 ID를 제외합니다).\n템플릿에 오류가 있으면 기본값 {post_id}_{post_title} 가 사용됩니다.\n참고: 이는 폴더 구조가 '게시물별 폴더'로 설정된 경우에만 적용됩니다.",
                "chinese-simplified": "可以在文件夹名称模板中使用占位符:\n{post_title}, {post_id}, {creator_name}, {creator_id}\n示例: {post_title}（从文件夹名称中删除帖子ID）。\n如果模板出错，将使用回退 {post_id}_{post_title}。\n注意: 仅当文件夹结构设置为“按帖子创建子文件夹”时才适用。",
            },
            "folder_name_template_error": {
                "english": "Folder name template failed to format, using fallback: {0}",
                "japanese": "フォルダ名テンプレートのフォーマットに失敗したため、フォールバックを使用します: {0}",
                "korean": "폴더 이름 템플릿 형식 지정에 실패하여 폴백을 사용합니다: {0}",
                "chinese-simplified": "文件夹名称模板格式化失败，使用回退: {0}",
            },
            "skip_posts_keywords": {
                "english": "Skip Posts Containing",
                "japanese": "除外するキーワード",
                "korean": "제외할 키워드",
                "chinese-simplified": "跳过包含以下内容的帖子",
            },
            ...
            "skip_posts_keywords_help_title": {
                "english": "Skip Posts Help",
                "japanese": "投稿除外のヘルプ",
                "korean": "게시물 제외 도움말",
                "chinese-simplified": "跳过帖子帮助",
            },
            "skip_posts_keywords_help_text": {
                "english": "Comma-separated keywords (e.g. AlienComic, sketch). Posts matching any keyword (case-insensitive, whole word) are excluded entirely.\n\nUse the 'Filter By' checkboxes to choose which fields are checked. Any combination can be enabled at once — a post is skipped if the keyword matches in ANY enabled field:\n  • Title — checks the post title\n  • Filenames — checks the main file and attachment names\n  • Description — checks the post body text\n  • Tags — checks the post's tags\n\nMatching is case-insensitive and whole-word. For example, 'sketch' matches 'Sketch' but not 'SketchBook'. To match multiple variations, enter them as separate keywords: 'AlienComic, Alien Comic, alien_comic'.\n\nLeave the keyword box blank to disable.",
                "japanese": "カンマ区切りのキーワード（例: AlienComic, sketch）。いずれかのキーワードに一致する投稿（大文字小文字を区別しない、単語単位）は完全に除外されます。\n\n「フィルター対象」チェックボックスで、どのフィールドをチェックするかを選択します。複数を同時に有効にでき、有効ないずれかのフィールドでキーワードが一致した投稿はスキップされます：\n  • タイトル — 投稿タイトルをチェック\n  • ファイル名 — メインファイルと添付ファイル名をチェック\n  • 説明 — 投稿本文をチェック\n  • タグ — 投稿のタグをチェック\n\n一致判定は大文字小文字を区別せず、単語単位で行われます。例: 'sketch' は 'Sketch' に一致しますが、'SketchBook' には一致しません。複数の表記に対応するには、'AlienComic, Alien Comic, alien_comic' のように別々のキーワードとして入力してください。\n\nキーワード欄を空にすると無効になります。",
                "korean": "쉼표로 구분된 키워드 (예: AlienComic, sketch). 키워드와 일치하는 게시물(대소문자 구분 없음, 단어 단위)은 완전히 제외됩니다.\n\n'필터 기준' 체크박스로 검사할 필드를 선택하세요. 여러 조합을 동시에 활성화할 수 있으며, 활성화된 필드 중 하나라도 키워드가 일치하면 게시물이 건너뛰어집니다:\n  • 제목 — 게시물 제목 확인\n  • 파일명 — 메인 파일 및 첨부 파일 이름 확인\n  • 설명 — 게시물 본문 텍스트 확인\n  • 태그 — 게시물 태그 확인\n\n일치 여부는 대소문자를 구분하지 않고 단어 단위로 판단됩니다. 예: 'sketch'는 'Sketch'와 일치하지만 'SketchBook'과는 일치하지 않습니다. 여러 표기를 대응하려면 'AlienComic, Alien Comic, alien_comic'처럼 개별 키워드로 입력하세요.\n\n키워드 입력란을 비우면 비활성화됩니다.",
                "chinese-simplified": "逗号分隔的关键词（例如: AlienComic, sketch）。匹配任一关键词的帖子（不区分大小写，整词匹配）将被完全排除。\n\n使用“过滤依据”复选框选择要检查的字段。可以同时启用任意组合——只要在任一启用的字段中匹配到关键词，帖子就会被跳过：\n  • 标题 — 检查帖子标题\n  • 文件名 — 检查主文件和附件名称\n  • 描述 — 检查帖子正文文本\n  • 标签 — 检查帖子的标签\n\n匹配不区分大小写，且按整词匹配。例如，'sketch' 能匹配 'Sketch'，但不会匹配 'SketchBook'。要匹配多种写法，请将它们作为独立关键词输入：'AlienComic, Alien Comic, alien_comic'。\n\n将关键词输入框留空即可禁用。",
            },
            "skip_files_keywords": {
            ...
            "posts_skipped_by_keyword": {
                ...
                "japanese": "除外キーワードに一致する投稿を {0} 件スキップしました: {1}",
            ...
            "skip_scope_title_toggle": {
                "english": "Title",
                "japanese": "タイトル",
                "korean": "제목",
                "chinese-simplified": "标题",
            },
            "skip_scope_filenames_toggle": {
                "english": "Filenames",
                "japanese": "ファイル名",
                "korean": "파일명",
                "chinese-simplified": "文件名",
            },
            "skip_scope_description_toggle": {
                "english": "Description",
                "japanese": "説明",
                "korean": "설명",
                "chinese-simplified": "描述",
            },
            "skip_scope_tags_toggle": {
                "english": "Tags",
                "japanese": "タグ",
                "korean": "태그",
            ...
            "date_range_filter": {
                ...
                "japanese": "日付範囲",
                "korean": "날짜 범위",
                "chinese-simplified": "日期范围",
            },
            "date_range_from": {
                "english": "From",
                "japanese": "開始日",
                "korean": "시작일",
                "chinese-simplified": "开始日期",
            },
            "date_range_from_placeholder": {
                "english": "YYYY-MM-DD (optional)",
                "japanese": "YYYY-MM-DD（任意）",
                "korean": "YYYY-MM-DD (선택 사항)",
                "chinese-simplified": "YYYY-MM-DD（可选）",
            },
            "date_range_to": {
                "english": "To",
                "japanese": "終了日",
                "korean": "종료일",
                "chinese-simplified": "结束日期",
            },
            ...
            "date_filter_help_title": {
                "english": "Post Date Filter Help",
                "japanese": "投稿日付フィルターのヘルプ",
                "korean": "게시물 날짜 필터 도움말",
                "chinese-simplified": "帖子日期过滤帮助",
            },
            "date_filter_help_text": {
                "english": "Filter posts by their own publish date (not the date they were imported/scraped). Enter dates as YYYY-MM-DD in 'From' and/or 'To' — both bounds are inclusive and both are optional; leave either blank to disable that bound. Posts with an unknown publish date are always kept.",
                "japanese": "投稿を、インポート/スクレイピング日ではなく投稿自身の公開日でフィルタリングします。'開始日' と '終了日' に YYYY-MM-DD 形式で日付を入力してください。両方の境界はその日を含み、どちらも任意です。一方を空にするとその境界は無効になります。公開日が不明な投稿は常に保持されます。",
                "korean": "게시물을 가져오기/스크랩 날짜가 아닌 게시물 자체의 게시 날짜로 필터링합니다. '시작일' 및/또는 '종료일'에 YYYY-MM-DD 형식으로 날짜를 입력하세요. 두 경계 모두 해당 날짜를 포함하며 둘 다 선택 사항입니다. 하나를 비워 두면 해당 경계가 비활성화됩니다. 게시 날짜를 알 수 없는 게시물은 항상 유지됩니다.",
                "chinese-simplified": "按帖子自身的发布日期（而非导入/抓取日期）过滤帖子。请在“开始日期”和/或“结束日期”中以 YYYY-MM-DD 格式输入日期——两个边界均包含当天且均为可选；将任一边界留空即可禁用该边界。发布日期未知的帖子将始终保留。",
            },
            "invalid_date_from_ignored": {
                "english": "Invalid 'Date Range: From' value ignored (use YYYY-MM-DD)",
                "japanese": "無効な「日付範囲: 開始日」の値は無視されました（YYYY-MM-DD を使用してください）",
                "korean": "잘못된 '날짜 범위: 시작일' 값이 무시되었습니다 (YYYY-MM-DD 사용)",
                "chinese-simplified": "无效的“日期范围：开始日期”值已忽略（请使用 YYYY-MM-DD）",
            },
            "invalid_date_to_ignored": {
                "english": "Invalid 'Date Range: To' value ignored (use YYYY-MM-DD)",
                "japanese": "無効な「日付範囲: 終了日」の値は無視されました（YYYY-MM-DD を使用してください）",
                "korean": "잘못된 '날짜 범위: 종료일' 값이 무시되었습니다 (YYYY-MM-DD 사용)",
                "chinese-simplified": "无效的“日期范围：结束日期”值已忽略（请使用 YYYY-MM-DD）",
            },
            "posts_skipped_by_date_filter": {
                "english": "Skipped {0} post(s) outside the selected date range.",
                "japanese": "選択した日付範囲外の投稿を {0} 件スキップしました。",
                "korean": "선택한 날짜 범위를 벗어난 게시물 {0}개를 건너뛰었습니다.",
                "chinese-simplified": "已跳过 {0} 个超出所选日期范围的帖子。",
            },
            "skipped_individual_file": {
                "english": "Skipped individual file (keyword match): {0}",
                "japanese": "個別ファイルをスキップしました（キーワード一致）: {0}",
                "korean": "개별 파일을 건너뛰었습니다 (키워드 일치): {0}",
                "chinese-simplified": "已跳过单个文件（关键词匹配）: {0}",
            },
            "skip_folder_for_few_images": {
                "english": "Skip Post Folder",
                "japanese": "投稿フォルダをスキップ",
                "korean": "게시물 폴더 건너뛰기",
                "chinese-simplified": "跳过帖子文件夹",
            },
            "skip_folder_info_title": {
                "english": "Skip Post Folder",
                "japanese": "投稿フォルダをスキップ",
                "korean": "게시물 폴더 건너뛰기",
                "chinese-simplified": "跳过帖子文件夹",
